...
class NB_model():
    ...
    def fit(self, train_data, train_label, vocab, if_use_smooth=True):
        ...
        self.Pr_dict = df_train.groupby(['classIdx', 'wordIdx']).sum().reset_index().pivot('wordIdx', 'classIdx', 'count').fillna(0).to_dict()
        for i in range(1, self.num_classes+1):
            sum = 0
            for j in range(1, self.num_vocab+1):
                if if_use_smooth:
                    self.Pr_dict[i][j] = self.Pr_dict[i].setdefault(j, 0) + 1
                else:
                    self.Pr_dict[i].setdefault(j, 0)
                sum += self.Pr_dict[i][j]
            for j in range(1, self.num_vocab+1):
                self.Pr_dict[i][j] /= float(sum)
    ...
    def predict(self, test_data):
        ...
        for docIdx in range(1, len(new_dict)+1):
            ...
            for classIdx in range(1,self.num_classes+1):
                ...
                for word, count in new_dict[docIdx].items():
                    score_dict[classIdx] += count * math.log(self.Pr_dict[classIdx][word])
